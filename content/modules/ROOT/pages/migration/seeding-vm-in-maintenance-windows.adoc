...
* Use: `import_workloads_cbt_sync: true`, `import_workloads_cutover: false`.
...
* Use: `import_workloads_cbt_sync: true` (or `false` to skip delta sync), `import_workloads_cutover: true`.
...
export GOVC_DATACENTER='RS00'
export GOVC_DATASTORE='workload_share_QHFNI'
...
VM_NAME="rhel4cbt"
FOLDER="ocpvirt-{guid}"
./govc vm.clone -vm=rhel93-tpl-l -folder="$FOLDER" -on=false -annotation='Created via govc' "$VM_NAME"
...
. Save the node. This is the first step of the workflow.

*Add the Approval node:*

. Click the 3 vertical dots on the *VM Seeding (Incremental Sync)* node click on *Add Step and link*.
. Select *Approval*.
. Configure the approval (e.g. name: "Approve cutover", add instructions such as "Confirm tenant is in maintenance window and ready for outage").
. Save the node.
...
. Ensure the *VM Cutover (Final Cutover)* node connects to *Success* (or the end node). If your UI uses an explicit *Success* node, connect the cutover node to it.
...
4. The *VM Seeding (Incremental Sync)* job runs first. Wait for it to complete (this simulates a maintenance window with no impact to the workload).
...
9. After approval, the *VM Cutover (Final Cutover)* job runs. This simulates the maintenance window where the tenant has an outage. Wait for it to complete.
...
openstack server add floating ip rhel4cbt {FLOATING_IP}
----
...
[source,bash,role=execute]
----
openstack server delete rhel4cbt
openstack volume delete rhel4cbt-2000
openstack port delete rhel4cbt-NIC-0-VLAN-private
----
