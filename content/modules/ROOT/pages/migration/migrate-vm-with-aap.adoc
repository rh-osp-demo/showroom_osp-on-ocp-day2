...
[source,bash,role=execute,subs=attributes]
----
echo $REG_ROUTE/$PROJECT/rhospvmt-ee:latest
----
...
2. Click *Create Project* and set the following parameters:
   ...
   * **Source Control URL:** `https://github.com/pnavarro/vmware-migration-kit`
   * **Source Control Branch/tag/commit:** `pnavarro-stable`
...
[source,bash,role=execute,subs=attributes]
----
openstack server delete haproxy_{guid}
openstack volume delete haproxy_{guid}-2000
openstack port delete haproxy_{guid}-NIC-0-VLAN-private
----
